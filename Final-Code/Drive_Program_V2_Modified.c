...
#pragma config(Servo,  srvo_S1_C4_4,    poleServo,            tServoStandard)
...
task main()
{
	...
	while (true) {
		...
		if (joystick.joy1_TopHat == 4) {
			//scoring position
			servo[trailerServoL] = 0;
			servo[trailerServoR] = 255;
		}
...
task liftLPresets() {
	while (true) {
		getJoystickSettings(joystick);
		//highest setting
		if (joy2Btn(3) && nMotorEncoder[liftL] < 4200) {
			while (nMotorEncoder[liftL] < 4200) {
